            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="row row-cols-lg-auto g-3 align-items-center justify-content-center mb-3"
                          action="{{route('sale.reference.report')}}" method="post">
                        @csrf
                        <div class="col-12">
...
{{--                                class="font-size-24 text-danger">{{$salesReturns ? $salesReturns->sum('amount') : 0}}</strong>--}}
                        ...
                        <div class="col-md-4">
                            ...
                            <strong class="font-size-24 text-success">
{{--                                {{($sales ? $sales->sum('amount') : 0) - ($salesReturns ? $salesReturns->sum('amount') : 0)}}--}}
                            </strong>
                        </div>
                    </div>
                        ...
                        <table id="datatable-buttons"
                            ...
                            @forelse($customerSales as $customerSale)
                                <tr>
                                    ...
                                    <td>{{$customerSale->date}}</td>
                                    <td>{{$customerSale->shop_name}}</td>
                                    <td>{{$customerSale->phone}}</td>
                                    <td>{{$customerSale->sales->sum('discount')}}</td>
                                    <td>{{$customerSale->sales->sum('amount')}}</td>
                                    <td>{{$customerSale->created_by}}</td>
                                </tr>
                            @empty
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="datatable-buttons"
                            ...
                            <tbody>
                            @forelse($customerSaleReturns as $customerSaleReturn)
                                <tr>
                                    <th scope="row">{{++$loop->index}}</th>
                                    <td>{{$customerSaleReturn->name}}</td>
                                    <td>{{$customerSaleReturn->shop_name}}</td>
                                    <td>{{$customerSaleReturn->phone}}</td>
                                    <td>{{$customerSaleReturn->saleReturns->sum('amount')}}</td>
                                    <td>{{$customerSaleReturn->created_by}}</td>
                                </tr>
                            @empty
                                <tr class="text-center">
