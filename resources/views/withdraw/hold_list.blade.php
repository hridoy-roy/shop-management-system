                                    ...
                                    <div class="dropdown text-center">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li><a href="{{route('withdraw.edit',$withdraw->id)}}" class="dropdown-item"><i
                                                    ...
                                                    Edit</a></li>
                                            <li>
                                                <a href="#" onclick="holdConfirm({{$withdraw->id}})" class="dropdown-item"><i
    ...
    <script>
        ...
        function holdConfirm(Id) {
            Swal.fire({
                ...
                t.value ?  $.ajax({
                    url: "{{ url('withdraw/hold/confirm') }}" + "/" + Id,
                    type: "post",
                    data: {
                        "_token": "{{ csrf_token() }}",
                    },
                    success: function (response) {
                        Swal.fire({
                            title: "Confirmed!",
                            text: "Withdraw Hole Info has been Confirm.",
                            icon: "success",
                        });
                        setTimeout(location.reload.bind(location), 2000);
                    },
                    error: function (response) {
                        Swal.fire({
                            title: "Fail",
                            text: "Withdraw Hold Confirm Fail",
                            icon: "error"
                        });
                    }
                }): t.dismiss === Swal.DismissReason.cancel && Swal.fire({
                    title: "Cancelled",
                    text: "Withdraw Hold Confirm Cancelled",
                    icon: "error"
                });
            });
        }
